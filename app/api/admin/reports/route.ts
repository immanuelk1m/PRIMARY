...
import { getReportsForAdmin } from '@/services/admin.service'; // 관리자 서비스 함수 import
...
export async function GET(request: Request) {
  ...
  const status = url.searchParams.get('status') as Enums<'report_status'> | undefined; // 타입 단언

  // 4. 관리자 권한 확인 후 서비스 함수 호출
  try {
    const { reports, count } = await getReportsForAdmin({ page, limit, status });
